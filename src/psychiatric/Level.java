...
public class Level {
    ...
    public void pickUpObject(int x, int y) {
        if (actualRoom != null) {
            // Intenta recoger el objeto en las coordenadas especificadas
            Object pickedObject = actualRoom.getObjectAt(x, y);

            if (pickedObject != null && pickedObject instanceof Inventory.Object) {
                Inventory.Object inventoryObject = (Inventory.Object) pickedObject;
                // Agregar el objeto recogido al inventario del jugador
                actualRoom.getPlayer().getInventory().addObject(inventoryObject);
                // Remover el objeto de la habitación
                actualRoom.removeObject(inventoryObject);
            }
        }
    }
